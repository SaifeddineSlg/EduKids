...
function buildPlaceholderLesson(subjectId: (typeof SUBJECT_ORDER)[number]): SubjectDayLesson {
  return {
    subjectId,
    status: "coming-soon",
...
export function buildPlaceholderDays(fromDay: number, toDay: number): DayCurriculum[] {
  ...
  for (let dayNumber = fromDay; dayNumber <= toDay; dayNumber += 1) {
    days.push({
      dayNumber,
      status: "coming-soon",
      subjects: SUBJECT_ORDER.map((subjectId) => buildPlaceholderLesson(subjectId))
    });
  }
