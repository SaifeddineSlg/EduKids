...
export async function PATCH(request: Request) {
  ...
  if (payload.thresholdValue !== undefined) updates.threshold_value = payload.thresholdValue;
  if (payload.isActive !== undefined) updates.is_active = payload.isActive;

  const supabase = getSupabaseServerClient();
  const { error } = await supabase.from("badge_definitions").update(updates).eq("id", payload.id);
