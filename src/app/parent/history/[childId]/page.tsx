...
export default function HistoryPage({ params }: HistoryPageProps) {
  ...
  useEffect(() => {
    params.then((resolved) => setChildId(resolved.childId));
  }, [params]);

  useEffect(() => {
    if (!childId) return;
    fetchDayAttempts(childId).then(setAttempts).catch(() => setAttempts([]));
  }, [childId]);
  ...
  useEffect(() => {
    if (!childId || !selectedDay || !selectedAttempt || !selectedSubject) return;
    fetchDayHistory(childId, selectedDay, selectedAttempt, selectedSubject)
      .then(setQuestions)
      .catch(() => setQuestions([]));
  }, [childId, selectedDay, selectedAttempt, selectedSubject]);
